...
//			var isFirstDataPointInPlotArea = true;
	...
	Chart.prototype.renderColumn = function(plotUnit){
		...
		var maxBarWidth = (this.canvas.width * 0.15);
		var xMinDiff = plotUnit.axisX.dataInfo.minDiff;
		var barWidth = (((plotArea.width / Math.abs(plotUnit.axisX.maximum - plotUnit.axisX.minimum)) * Math.abs(xMinDiff)) / plotUnit.plotType.totalDataSeries * 0.9) << 0;
	...
	Chart.prototype.renderStackedColumn = function(plotUnit){
		...
		var maxBarWidth = this.canvas.width * 0.15;
		var xMinDiff = plotUnit.axisX.dataInfo.minDiff;
		var barWidth = (((plotArea.width / Math.abs(plotUnit.axisX.maximum - plotUnit.axisX.minimum)) * Math.abs(xMinDiff)) / plotUnit.plotType.plotUnits.length * 0.9) << 0;
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++){
			...
			if (dataPoints.length > 0){
				...
				for (i = 0; i < dataPoints.length; i++){
					...
					if (dataPoints[i].y >= 0){
						offset = offsetPositiveY[dataPointX] ? offsetPositiveY[dataPointX] : 0;
						...
						offset = offsetNegativeY[dataPointX] ? offsetNegativeY[dataPointX] : 0;
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++){
			...
			if (dataPoints.length > 0){
				...
				for (i = 0; i < dataPoints.length; i++){
					...
					y = (plotUnit.axisY.convertionParameters.reference + plotUnit.axisY.convertionParameters.pixelPerUnit * (yPercent - plotUnit.axisY.convertionParameters.minimum) + 0.5) << 0;
	...
	Chart.prototype.renderBar = function(plotUnit){
		...
		var maxBarWidth = this.canvas.height * 0.15;
		var xMinDiff = plotUnit.axisX.dataInfo.minDiff;
		...
		var barWidth = (((plotArea.height / Math.abs(plotUnit.axisX.maximum - plotUnit.axisX.minimum)) * Math.abs(xMinDiff)) / plotUnit.plotType.totalDataSeries * 0.9) << 0;
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++){
			...
			if (dataPoints.length > 0){
				...
				for (i = 0; i < dataPoints.length; i++){
					...
					y = (plotUnit.axisX.convertionParameters.reference + plotUnit.axisX.convertionParameters.pixelPerUnit * (dataPointX - plotUnit.axisX.convertionParameters.minimum) + 0.5) << 0;
					x = (plotUnit.axisY.convertionParameters.reference + plotUnit.axisY.convertionParameters.pixelPerUnit * (dataPoints[i].y - plotUnit.axisY.convertionParameters.minimum) + 0.5) << 0;
	...
	Chart.prototype.renderStackedBar = function(plotUnit){
		...
		var maxBarWidth = this.canvas.width * 0.15;
		var xMinDiff = plotUnit.axisX.dataInfo.minDiff;
		var barWidth = (((plotArea.height / Math.abs(plotUnit.axisX.maximum - plotUnit.axisX.minimum)) * Math.abs(xMinDiff)) / plotUnit.plotType.plotUnits.length * 0.9) << 0;
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++){
			...
			if (dataPoints.length > 0){
				...
				for (i = 0; i < dataPoints.length; i++){
					...
					x = (plotUnit.axisY.convertionParameters.reference + plotUnit.axisY.convertionParameters.pixelPerUnit * (yPercent - plotUnit.axisY.convertionParameters.minimum) + 0.5) << 0;
	...
	Chart.prototype.renderStackedArea100 = function(plotUnit){
		...
		var maxBarWidth = this.canvas.width * 0.15;
		var xMinDiff = plotUnit.axisX.dataInfo.minDiff;
		var barWidth = (((plotArea.width / Math.abs(plotUnit.axisX.maximum - plotUnit.axisX.minimum)) * Math.abs(xMinDiff)) * 0.9) << 0;
	...
	Chart.prototype.renderBubble = function(plotUnit){
		...
		var maxBarWidth = this.canvas.width * 0.15;
		var xMinDiff = plotUnit.axisX.dataInfo.minDiff;
		var barWidth = (((plotArea.width / Math.abs(plotUnit.axisX.maximum - plotUnit.axisX.minimum)) * Math.abs(xMinDiff)) / totalDataSeries * 0.9) << 0;
		...
		var maxArea = Math.max(Math.pow(Math.min(plotArea.height, plotArea.width) * 0.25 / 2, 2) * Math.PI, minArea);
	...
	Chart.prototype.renderScatter = function(plotUnit){
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++){
			...
			if (dataPoints.length > 0){
				...
				//var maxArea = Math.pow(Math.min(plotArea.height, plotArea.width) * 0.3 / 2, 2) * Math.PI;
	...
	Chart.prototype.renderPie = function(plotUnit){
		...
		var startAngle = 'startAngle' in dataSeries ? dataSeries.startAngle : 0;
		...
		for (var i = 0; i < noDataPoints; i++){
			...
			labelText = this.replaceKeywordsWithValue(labelText, dataSeries.dataPoints[i], dataSeries, i);
		...
		if (dataSeries.indexLabelPlacement === "inside"){
			...
			for (i = 0; i < noDataPoints; i++){
				...
				dx = labelRadius * (Math.cos((labelLocationAngles[i])));
				dy = labelRadius * (Math.sin((labelLocationAngles[i])));
				x1.push(dx);
				if (dx > 0)
		...
		function animate(){
			...
			var fps = (that.animationEnabled === true) ? 60 : 1;
			...
			if (animationParameter != null && animationParameter.frame <= fps){
				...
				for (var i = 0; i < noDataPoints; i++){
					...
					var endAngle = startAngle + ((2 * Math.PI / total) * Math.abs(dataSeries.dataPoints[i].y));
					...
					if (dataSeries.indexLabelPlacement === "outside"){
						...
						var indexLineMinLength = maxAvail / 14;
		...
		function implode(){
			...
			if (animationParameter !== null && animationParameter.frame > 0){
				...
				for (var i = 0; i < noDataPoints; i++){
					...
					currentlyExploded = that._eventManager.objectMap[id].currentlyExploded;
					var Xoffset = radius * explodeOffset * (Math.cos((labelLocationAngles[i])));
					var Yoffset = radius * explodeOffset * (Math.sin((labelLocationAngles[i])));
